...
class CentralWidget(qtw.QWidget):
    ...
    def __init__(self, connection):
        ...
        self.graph_accuracy_que1 = deque(maxlen=self.master_data_width)
        self.graph_accuracy_que2 = deque(maxlen=self.master_data_width)
        self.graph_accuracy_que3 = deque(maxlen=self.master_data_width)
        self.graph_accuracy_que4 = deque(maxlen=self.master_data_width)
        self.graph_accuracy_lineque = deque(maxlen=self.master_data_width)
        self.graph_accuracy = RewardPlotWidget(self, plot_layers=3, graph_label='Agent Accuracy',
            label_1="Agent Accuracy", label_2="Avg(25)", label_3="Learning Session",
            y_min=40, y_max=100, left=0.04
            )
        ...
        self.total_reward_graph = self.graph_accuracy
        ...
        self.central_widget_layout.addWidget(self.action_module(), 1, 0, 1, 2)
        self.central_widget_layout.addWidget(self.img_module(self.image_graph), 1, 2, 1, 2)
        self.central_widget_layout.addWidget(self.bar_module(self.action_graph), 1, 4, 1, 2)        
        self.central_widget_layout.addWidget(self.metrics_module(), 1, 6, 1, 12)
        
        self.central_widget_layout.addWidget(self.curve_module(self.actor_critic_graph), 2, 0, 1, 6)
        self.central_widget_layout.addWidget(self.plot_module(self.graph_action_failrate), 2, 6, 1, 12)

        self.central_widget_layout.addWidget(self.plot_module(self.graph_avg_qmax), 3, 0, 1, 6)
        self.central_widget_layout.addWidget(self.plot_module(self.total_reward_graph),3, 6, 1, 12)

        self.central_widget_layout.addWidget(self.new_button("quit_button"), 4, 4, 1, 2)
        self.central_widget_layout.addWidget(self.new_button("fast_button"), 4, 6, 1, 4)
    ...
    def metrics_module(self):
        ...
        self.session_accuracy = qtw.QLabel(f' Session Accuracy: -', self)
        self.session_accuracy.setFont(self.metric_font)
        self.session_accuracy.setStyleSheet(metric_style)
        ...
        self.total_reward = qtw.QLabel(f' Fail Streak Triple: -/-/-', self)
        ...
        self.critic_value = qtw.QLabel(f' -/-', self)
        self.critic_value.setFont(self.metric_font)
        self.critic_value.setStyleSheet(metric_style)
        self.move_tracker = qtw.QLabel(f' Move Fail Rate: -', self)
        self.move_tracker.setFont(self.metric_font)
        self.move_tracker.setStyleSheet(metric_style)
        ...
        metrics_grid.addWidget(self.session_accuracy,4,0,1,1)
        ...
        metrics_grid.addWidget(self.critic_value,3,1,1,1)
        metrics_grid.addWidget(self.avg_qmax,4,1,1,1)
        metrics_grid.addWidget(self.move_tracker,5,1,1,1)
    ...
    def update_gui(self):
        ...
        try:
            ...
            self.last_reset.setText(f' Subcycle/Frame: {self.metrics[4]}/{self.metrics[23][0]}/{self.metrics[23][1]}%')
            self.critic_value.setText(f'__________________________________________________________')
            ...
            self.move_tracker.setText(f' Move Fail Rate: {move_tracker_items}')
            self.performance_trend.setText(f' Accuracy Trend: {self.metrics[10]}%')
            self.cycles_per_second.setText(f' Cycles/Second: {self.metrics[12]}')
            self.avg_qmax.setText(f' Actor/Critic Avg Qmax: {self.metrics[13]}/{self.metrics_last[24]}')

            ##Only the metrics module is updated per-frame in fast-mode.
            if not self.fast_mode:
                ...
                #image modulemetrics[22] = img directory, frame#
                if (self.metrics[22] != self.metrics_last[22]):
                    self.image_graph.change_dataset(self.metrics[22])
                if self.metrics[4] % 2==0: #only update every n many subcycles.
                    self.image_graph.tick_frame(self.metrics[23][0])
                ...
                if self.metrics[1]>0: #Agent guessed correctly.
                    color_set[action] = int(2)
                if self.metrics[1]<=0: #Agent guessed incorrectly.
            ...
            if (self.metrics[20] != self.metrics_last[20]) and (self.metrics[6] > self.metrics[18]): #Once per game
                self.graph_accuracy_que1.append(self.metrics[16]) #performance
                self.graph_accuracy_que2.append(self.metrics[10]) #avg

                """ Not using the learning bool function currently. when used, it adds a vertical line to the plot at that spot.
                #if (self.metrics[20]-1)%self.metrics[18] == 0:
                if self.metrics[21]: #self.metrics[21] no longer is learning bool, find a new var.
                    self.graph_accuracy_lineque.append(1)
                else:
                    self.graph_accuracy_lineque.append(0)  
                """
                self.graph_accuracy_lineque.append(0)
                self.total_reward_graph.add_value(self.graph_accuracy_que1, self.graph_accuracy_que2, self.graph_accuracy_lineque)
                ...
                self.graph_avg_qmax_que3.append(self.metrics_last[24]) #critic avg
...
class PlotWidget(qtw.QWidget):
    ...
    def add_value(self, input_1, input_2=[0], input_3=[0], input_4=[0]):
        ...
        if self.y_min<0:
            self.y_min *= 1.25 #negatives have to scale more negative for spacing.
...
class QuickImageWidget(qtw.QWidget):
    ...
    def __init__(self, *args, minimum=0, maximum=250, **kwargs):
        ...
        starter_img = cv2.imread("Resources\screenshot.png", cv2.IMREAD_UNCHANGED) #cv2.IMREAD_GRAYSCALE
        self.frame_buffer = []
