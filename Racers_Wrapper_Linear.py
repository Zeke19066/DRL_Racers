...
from Custom_Keymapper import pybind11module as key_mapper #Custom C++ keymap library (Fast)
...
class ScreenGrab():
    ...
    def resize(self, img, wrong_way_bool):
        xy = int(504/4)
        img = np.array(img)
    ...
    def reward_scan(self, image, mode):
        """
...
class Actions():
    ...
    def __init__(self, reward_mode):
        ...
        self.mode, self.course, self.track = 4, 1, 4 #mode/course/track selection.

    # Map NN output to actions
    def action_Map(self, action):
        #Note: Minmap mode doesnt update repeats and power up includes forward. Speed does not do this.
        #key_mapper.actionMapMinmapDrift(action, self.last_action)

        key_mapper.actionMapSpeed(action, self.last_action)
    ...
    def quit_Reset(self):
        #self.course, self.track = self.random_track()
        if self.reward_mode == 0:
            key_mapper.quitResetSpeed(self.mode, self.course, self.track)
        elif self.reward_mode == 1:
            key_mapper.quitResetMinmap(self.mode, self.course, self.track)

    def first_Run(self):
        #self.course, self.track = self.random_track()

        if self.reward_mode == 0:
            key_mapper.firstRunSpeed(self.mode, self.course, self.track)
        elif self.reward_mode == 1:
            key_mapper.firstRunMinmap(self.mode, self.course, self.track)
    ...
    def random_track(self):
        course = np.random.randint(1,4)
        track = np.random.randint(1,5)

        #let's avoid the snow level, which throws off the speed guage.
        if course == 2:
            while track == 3:
                track = np.random.randint(1,5)
        return course, track
